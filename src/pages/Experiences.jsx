...
const ExperienceCard = ({ post }) => {
  ...
  const startDate = parseCustomDate(post.metadata?.startDate) || new Date(0);
  const endDateRaw = post.metadata?.endDate?.toLowerCase() || '';
  const isPresent = !endDateRaw || endDateRaw === 'present' || endDateRaw === 'ongoing';
  const endDate = isPresent ? null : parseCustomDate(post.metadata.endDate);

  const formattedStart = formatDateAsMonthYear(startDate);
  const formattedEnd = isPresent ? (
    <span className="italic text-blue-600">Present</span>
  ) : (
    formatDateAsMonthYear(endDate)
  );
  const durationText = calculateDuration(startDate, endDate || new Date());

  return (
    <div className="flex flex-col md:flex-row items-start bg-white rounded-xl shadow-sm border mb-6 p-4 hover:shadow-lg transition-shadow duration-200">
      ...
      <div className="flex-1">
        ...
        {(startDate || endDate) && (
          <p className="text-xs text-gray-500 mb-2">
            <span
              title={`${formatDateAsDDMMYYYY(startDate)}${
                endDate ? ` – ${formatDateAsDDMMYYYY(endDate)}` : ''
              }`}
            >
              {formattedStart} – {formattedEnd}
            </span>
            <span className="ml-2 text-gray-400">({durationText})</span>
          </p>
        )}
...
const ExperiencePage = () => {
  ...
  const sortedPosts = Array.isArray(blogPosts)
    ? [...blogPosts].sort((a, b) => {
        const dateA = a.metadata?.startDate ? parseCustomDate(a.metadata.startDate) : new Date(0);
        const dateB = b.metadata?.startDate ? parseCustomDate(b.metadata.startDate) : new Date(0);
        return dateB - dateA;
      })
    : [];
